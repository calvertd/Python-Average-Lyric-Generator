...
def get_artist_songs(input_artist):
    ...
    try:
        result = musicbrainzngs.search_releases(artist=input_artist, limit=1)
        if result["release-list"][0]['artist-credit'][0]['name'].lower() == input_artist.lower():
            artist_id = result["release-list"][0]["id"]
        else:
            print("Cannot find artist specified")
            exit()
        
        #### get tracklist
        recordings = musicbrainzngs.get_release_by_id(artist_id, includes=["recordings"])
        song_list = []
